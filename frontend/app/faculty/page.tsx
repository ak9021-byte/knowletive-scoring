...
import { getStudents, createStudent, deleteStudent, getLeaderboard, getStudentOfDay, submitScore, giveReward, getAllRewards } from "@/lib/api"
...
export default function FacultyPage() {
  ...
  const fetchAll = async () => {
    try {
      ...
      setStudents(s.data); setLeaderboard(l.data); setRewards(r.data)
    } catch {}
  ...
  const avgScore = leaderboard.length>0 ? Math.round(leaderboard.reduce((a,b)=>a+b.total,0)/leaderboard.length) : 0
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        :root {
          ...
          --bg:#f8f9fe; --white:#ffffff; --sidebar:#ffffff;
        ...
        .faculty-badge {
          ...
          background:#f8f9fe; border-radius:var(--radius-sm); border:1px solid var(--border); margin-bottom:10px;
        }
        .faculty-avatar {
        ...
        .logout-btn {
          ...
          cursor:pointer; font-size:13px; font-weight:600; font-family:var(--font); transition:all 0.2s;
        }
        ...
        .stat-card {
          ...
          padding:22px 24px; box-shadow:var(--shadow); position:relative; overflow:hidden; transition:all 0.2s;
        }
        ...
        .sod-card {
          ...
          display:flex; align-items:center; gap:20px; box-shadow:0 4px 20px rgba(245,158,11,0.12);
        }
        ...
        .lb-row {
          ...
          margin-bottom:8px; transition:all 0.2s; box-shadow:var(--shadow);
        }
        ...
        .f-input {
          ...
          font-size:14px; outline:none; transition:all 0.2s; font-family:var(--font);
        }
        ...
        .score-metric {
          ...
          border-radius:12px; border:1.5px solid var(--border); background:var(--white); transition:all 0.2s;
        }
        ...
        .score-btns { display:flex; gap:6px; margin-left:auto; flex-wrap:wrap; }
        .score-btn {
          padding:7px 15px; border-radius:8px; border:1.5px solid var(--border);
          background:var(--white); color:var(--muted); cursor:pointer;
          font-size:13px; font-weight:700; font-family:var(--font); transition:all 0.15s;
        }
        .score-btn:hover { border-color:var(--accent); color:var(--accent); background:var(--accent-light); }
        .score-btn.active { background:var(--accent); border-color:var(--accent); color:#fff; box-shadow:0 3px 12px rgba(91,94,244,0.3); }
        .total-box {
          display:flex; align-items:center; justify-content:space-between;
          padding:20px 24px; border-radius:var(--radius-sm); border:1.5px solid var(--border);
          background:#fafbff; margin-bottom:18px;
        }
        ...
        .btn-gold {
          ...
          font-family:var(--font); transition:all 0.2s; box-shadow:0 4px 16px rgba(245,158,11,0.25);
        }
        ...
        .btn-del {
          ...
          font-size:13px; font-weight:600; font-family:var(--font); transition:all 0.2s;
        }
        ...
        .fu4 { animation-delay:0.2s; opacity:0; }
      `}</style>
      ...
      <div className="layout">
        ...
        <div className="mobile-bar">
          ...
          <button onClick={() => setSidebarOpen(!sidebarOpen)}
            style={{ background:"#f1f5f9", border:"1px solid var(--border)", borderRadius:8, padding:"8px 12px", cursor:"pointer", fontSize:18 }}>☰</button>
        </div>
            ...
            <div style={{ width:264, background:"#fff", height:"100%", padding:"80px 12px 24px", boxShadow:"4px 0 24px rgba(0,0,0,0.12)" }} onClick={e => e.stopPropagation()}>
              {tabs.map(t => (
                <button key={t.id} onClick={() => { setTab(t.id); setSidebarOpen(false) }}
                  className={`nav-item ${tab===t.id?"active":""}`} style={{ marginBottom:3 }}>
            ...
            <div style={{ maxWidth:880 }}>
              ...
              <div className="stats-row fu fu2">
                {[
                  ...
                  { label:"Scored Today", value:leaderboard.length, icon:"✅", accent:"#10b981", iconBg:"#ecfdf5" },
              ...
              {leaderboard.length>0 && (
                <div className="card fu fu3">
                  <div className="sec-label">🏆 Today's top performers</div>
                    ...
                    return (
                      <div key={i} className="lb-row" style={{ background:i===0?"linear-gradient(135deg,#fffbeb,#fef9c3)":"#fff", borderColor:i===0?"#fde68a":"var(--border)" }}>
                        ...
                        <div className="lb-bar-wrap">
                          <div className="lb-bar" style={{ width:`${e.total}%`, background:t.color }} />
                        </div>
          ...
          {tab==="leaderboard" && (
            <div style={{ maxWidth:700 }}>
              <div className="page-header fu">
                ...
                <p className="page-sub">Today's student rankings by score</p>
              </div>
              {leaderboard.length===0 ? (
                <div className="card fu fu1" style={{ textAlign:"center", padding:"60px 24px" }}>
                  <div style={{ fontSize:52, marginBottom:14 }}>📭</div>
                  <p style={{ color:"var(--muted)", fontSize:16 }}>No scores submitted today yet</p>
                </div>
              ) : (
